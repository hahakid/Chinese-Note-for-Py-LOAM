...
if 1:
    ...
    w[1] = np.pi / 6
    w[2] = np.pi / 6
    R1=transforms3d.euler.euler2mat(w[0],w[1],w[2])
    #R2=mrob.geometry.SO3(R1)
    R = mrob.geometry.SO3(w)
    #print(R1)
    #R2.print()
    #R.print()
    rt=np.hstack([w,[0,0,0]])

    TT = mrob.geometry.SE3(rt)
    TT1 = np.eye(4)
    ...
    ax = plotConfig()
    plotT(mrob.geometry.SE3(TT), ax)
    plotT(mrob.geometry.SE3(TT1), ax)
